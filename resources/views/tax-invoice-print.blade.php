...
<head>
    ...
    <title>Tax Invoice - {{ $taxInvoice->invoice_no ?? 'TI-00001' }}</title>
    <style>
        /*
         * GLOBAL STYLES
         */
        ...
        <div class="header-content">
            ...
            <div class="quotation-details">
                <div class="customer-info">
                    <strong>{{ $taxInvoice->customer_name ?? 'Customer Name' }}</strong><br>
                    {{ $taxInvoice->customer_address ?? 'Customer Address' }}<br>
                    <span class="contact-label">Phone No.</span><span class="contact-separator">: </span><span class="contact-value">{{ $taxInvoice->customer_phone ?? 'Phone Number' }}</span><br>
                </div>
                <div class="quotation-meta">
                    <span class="quotation-label">Invoice No.</span><span class="quotation-separator">: </span><span class="quotation-value">{{ $taxInvoice->invoice_no ?? 'TI-00001' }}</span><br>
                    <span class="quotation-label">Case Ref</span><span class="quotation-separator">: </span><span class="quotation-value">{{ $taxInvoice->case->case_number ?? 'N/A' }}</span><br>
                    <span class="quotation-label">Issue Date</span><span class="quotation-separator">: </span><span class="quotation-value">{{ optional($taxInvoice->invoice_date)->format('d/m/Y') ?? '04/09/2025' }}</span><br>
                    <span class="quotation-label">Due Date</span><span class="quotation-separator">: </span><span class="quotation-value">{{ optional($taxInvoice->due_date)->format('d/m/Y') ?? '04/10/2025' }}</span><br>
            ...
            <div class="footer-top">
                <div class="left-section">
                    <div class="amount-in-words">
                        Ringgit Malaysia: {{ $taxInvoice->total_words ?? 'Amount in Words' }}
                    </div>
                    <div class="note-section">
                        <strong>Note:</strong> {{ $taxInvoice->remark ?? '' }}
                    </div>
                </div>
                ...
                <div class="right-section">
                    <table class="summary-table-bordered">
                        <tr>
                            ...
                            <td class="summary-value-bordered">{{ number_format($taxInvoice->subtotal ?? 0, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="summary-label-bordered">Tax</td>
                            <td class="summary-value-bordered">{{ number_format($taxInvoice->tax_total ?? 0, 2) }}</td>
                        </tr>
                        <tr>
                            <td class="summary-label-bordered">Total</td>
                            <td class="summary-value-bordered total-row">{{ number_format($taxInvoice->total ?? 0, 2) }}</td>
                        </tr>
                    </table>
                </div>
            </div>
