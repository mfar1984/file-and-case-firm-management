...
@section('breadcrumb')
    Settings > Firm Management > Create Firm
@endsection
...
<div class="px-4 md:px-6 pt-4 md:pt-6 pb-6 max-w-7xl mx-auto">
    <div class="bg-white rounded shadow-md border border-gray-300">
                ...
                <div>
                    <div class="flex items-center">
                        <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                            <span class="material-icons text-lg text-blue-600">add_business</span>
                        </div>
                        <div>
                            <h1 class="text-lg md:text-xl font-bold text-gray-800">Create New Firm</h1>
                            <p class="text-xs text-gray-500 mt-1">Add a new firm to the system</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="p-4 md:p-6">
            <form action="{{ route('settings.firms.store') }}" method="POST" enctype="multipart/form-data" class="space-y-0">
                @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" rows="3"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
                                      placeholder="Complete firm address...">{{ old('address') }}</textarea>
                            @error('address')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-2">Initial Setup</label>
                            <div class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-md text-xs text-gray-600">
                                New firm - no users yet
                            </div>
                        </div>
                    </div>
                </div>
